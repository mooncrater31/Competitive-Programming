...
const pair<int, int> dirs[nr_dirs] = {
  make_pair(1, 0), make_pair(-1, 0), make_pair(0, 1), make_pair(0, -1)};//for the floodfill algorithm
const char hieroglyph_characters[] = {0, 'W', 'A', 'K', 'J', 'S', 'D'};

const int H_max = 200, W_max = 50;//constraints
int image[H_max + 2][W_max * 4 + 2];//Why +2? 
...
int main()
{
  ...
  for (int case_nr = 1; ; case_nr++) {
    ...
    if (!H)
      break;
    for (int i = 1; i <= H; i++) {//Rows start from 1 
      char s[W_max + 1];
      scanf("%s", s);
      image[i][0] = 0;
      for (int j = 0, k = 1; j < W; j++, k += nr_pixels) {//columns start from 1 as well
        int p = (isdigit(s[j])) ? s[j] - '0' : s[j] - 'a' + 10;//Hexadecimal to decimal conversion 
        for (int l = 0, b = 8; l < nr_pixels; l++, b >>= 1)//pixel filling. 
          image[i][k + l] = (p & b) ? 1 : 0;
      }
      image[i][W * nr_pixels + 1] = 0;//why?
    }
    W = W * nr_pixels + 2;//change in W
    for (int j = 0; j < W; j++)
