...
class OliverAndTheGame
{
	...
	{
		if(!toNode.containsKey(a))
		{
			toNode.put(a,new nodeB(a)) ;
		}
		if(!toNode.containsKey(x))
		{
			// System.out.println(x+" was added");
			toNode.put(x,new nodeB(x)) ;
		}
	...
	void modifiedDFS(int a)
	{
		...
		if(toList.containsKey(a))
		{
			...
			nodeB Nb = toNode.get(a) ;
			
			for(nodeA tempA = Na;tempA!= null;tempA = tempA.next)
			{
				nodeB tempB = toNode.get(tempA.value) ;
				tempB.H.addAll(Nb.H) ;
